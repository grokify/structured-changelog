...
Both `sclog` and `structured-changelog` commands are available after installation.

### Go Install

```bash
go install github.com/grokify/structured-changelog/cmd/sclog@v0.3.0
```
...
Install `sclog` via Homebrew with a single command:
...
- `sclog` - Short form for daily use
