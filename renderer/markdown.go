...
func renderHeaderLine(cl *changelog.Changelog, l *messages.Localizer) string {
	var parts []string

	// Always include Keep a Changelog reference
	parts = append(parts, l.T("header.format_kacl"))
	...
	case changelog.VersioningSemVer:
		parts = append(parts, l.T("header.versioning_semver"))
	case changelog.VersioningCalVer:
		parts = append(parts, l.T("header.versioning_calver"))
	...
	if cl.CommitConvention == changelog.CommitConventionConventional {
		parts = append(parts, l.T("header.commits_conventional"))
	}

	// Always include Structured Changelog reference
	parts = append(parts, l.T("header.generated_by"))
	...
	for i, part := range parts {
		if i == 0 {
			...
			sb.WriteString(",\n")
			sb.WriteString(l.T("header.conjunction"))
			sb.WriteString(" ")
			sb.WriteString(part)
		} else {
			sb.WriteString(",\n")
